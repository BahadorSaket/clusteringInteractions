...
function getCoordination(element){
  ...
  // var yArray = [];
  //
  // for(var i=0;i<pointsInGroup.length;i++)
  // {
  //   string = d3.select("#"+pointsInGroup[i]).attr("transform");
  //   translate = string.substring(string.indexOf("(")+1, string.indexOf(")")).split(",");
  //
  //   xArray.push(Number(translate[0]));
  //   yArray.push(Number(translate[1]));
  // }
  //
  // var minX = Math.min(...xArray);
  // var minY = Math.min(...yArray);
  //
  // var maxX = Math.max(...xArray);
...
function drawBoundary(listOfGroups){
  d3.selectAll(".g_cluster").remove();


}

function attachCircles(listOfGroups, index){

  for(var k=0;k<listOfGroups[index].length;k++)
  {
    string = d3.select("#"+listOfGroups[index][k]).attr("transform");
    circleCoord= string.substring(string.indexOf("(")+1, string.indexOf(")")).split(",");

    string = d3.select("#g_cluster_"+index).attr("transform");
    groupCoord = string.substring(string.indexOf("(")+1, string.indexOf(")")).split(",");

    curCircleCoordX = Math.abs(Number(circleCoord[0]) - Number(groupCoord[0]));
    curCircleCoordY = Math.abs(Number(circleCoord[1]) - Number(groupCoord[1]));

    $("#"+listOfGroups[index][k]).appendTo("#g_cluster_"+index);

    d3.select("#"+listOfGroups[index][k])
      .attr("transform", function(){
          return "translate("+(curCircleCoordX + 2*Number(radius)+ Number(radius)/2)+","+(curCircleCoordY + 2*Number(radius))+")"
      });
  }
}

function drawRectangle(coordination, val){
